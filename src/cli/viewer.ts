...
async function viewThread(
  threadUrl: string,
  page: number = 1,
  subforumUrl: string | null = null
): Promise<void> {
  clearConsole()
  console.log(
    boxen(`${EMOJI_INFO} Viewing Thread:`, {
      padding: 1,
      margin: 1,
      borderStyle: 'double',
    })
  ...
  if (posts.length === 0) {
    ...
    const lastPost =
      posts.length > 0 ? formatDate(posts[posts.length - 1].postedAt) : 'N/A'

    console.log(
      `${EMOJI_INFO} Thread Stats: 👤 ${userCount}, 👀 Last Post: ${lastPost}\n`
    )
    ...
    for (const post of pagePosts) {
      ...
      for (const file of files) {
        if (file.mimeType?.startsWith('image')) {
          ...
          console.log(
            `  📎 Attached file: ${file.filename} (${file.mimeType || 'Unknown type'})`
          )
        }
      }
...
async function browseThreads(
  ...
  clearConsole()
  console.log(
    boxen(`${EMOJI_INFO} Browsing Threads:`, {
      padding: 1,
      margin: 1,
      borderStyle: 'double',
    })
  ...
  console.log(
    `${EMOJI_INFO} Subforum Stats: 💬 ${postCount}, 👤 ${userCount}\n`
  )
  ...
  for (const [index, thread] of pageThreads.entries()) {
    ...
    options.push(
      `${startIdx + index + 1}. ${thread.title} - ${thread.creator} (💬 ${postCount}) [${formatDate(thread.createdAt)}]`
    )
  }
...
async function browseSubforums(
  parentId: number | null = null,
  page: number = 1
): Promise<void> {
  clearConsole()
  console.log(
    boxen(`${EMOJI_INFO} Browsing Subforums:`, {
      padding: 1,
      margin: 1,
      borderStyle: 'double',
    })
  ...
  for (const [index, subforum] of pageSubforums.entries()) {
    const threadCount =
      parentId === null ? await getThreadsCountBySubforum(subforum.url) : 0
...
main().catch((error) =>
  console.error(`${EMOJI_ERROR} Unexpected error:`, error)
)
